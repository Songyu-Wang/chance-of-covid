...
for fips, state in LIST_OF_STATES_WITH_FIPS.items():
    ...
    prep_data['Possibility_1'].append(
        guess_current_covid_count / pop_data[state])
    prep_data['Possibility_10'].append(
        guess_current_covid_count / pop_data[state] * 10)
    prep_data['Possibility_100'].append(
...
df.to_html(open('data.html', 'w'), formatters={
    internal_to_external_name_map['Possibility_1']: '{:,.2%}'.format,
    internal_to_external_name_map['Possibility_10']: '{:,.2%}'.format,
    internal_to_external_name_map['Possibility_100']: '{:,.2%}'.format,
}, index=False)
